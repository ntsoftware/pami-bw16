...
# Parse a capture file and dump JSON output.
...
class Heartbeat:
    ...
    @classmethod
    def parse(cls, reader: Reader) -> "Heartbeat":
        robot_mode = RobotMode(reader.next_u8())
        team_color = TeamColor(reader.next_u8())
        goal_zone = reader.next_u8()
        game_time = reader.next_u32()
        return cls("heartbeat", robot_mode, team_color, goal_zone, game_time)
...
Frame: TypeAlias = Heartbeat | Move | Scan | EstimatedPose | CurrentPose | Path | Motor
...
class UnknownFrameType(Exception):
    def __init__(self, frame_type: int) -> None:
        super().__init__(f"unknown frame type {frame_type}")
...
def parse_frame(reader: Reader) -> Frame:
    ...
    if frame_type == FrameType.HEARTBEAT:
        return Heartbeat.parse(reader)
    elif frame_type == FrameType.MOVE:
        return Move.parse(reader)
    elif frame_type == FrameType.SCAN:
        return Scan.parse(reader)
    elif frame_type == FrameType.ESTIMATED_POSE:
        return EstimatedPose.parse(reader)
    elif frame_type == FrameType.CURRENT_POSE:
        return CurrentPose.parse(reader)
    elif frame_type == FrameType.PATH:
        return Path.parse(reader)
    elif frame_type == FrameType.MOTOR:
        return Motor.parse(reader)
    else:
...
parser = ArgumentParser(description="Parse a capture file and dump JSON output.")
parser.add_argument(
    "file", metavar="FILE", nargs="?", help="capture file (use stdin if empty)"
)
